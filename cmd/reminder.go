...
func NewReminderCmd() (*cobra.Command, error) {
	reminderCmd := &cobra.Command{
		...
		Example: fmt.Sprintf("bribot reminder --%s debug --%s https://discord.com/api/webhooks/id/pwd --%s punkctf--%s https://ctf.example.com --%s 2024-01-01 00:00:00 UTC --%s 2024-02-01 00:00:00 UTC --%s teamName --%s teamPassword",
			flagLogLevel,
			flagWebhookUrl,
			flagCTFName,
			flagCTFURL,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			challenge, err := challenge.NewChallenge(challengeName, challengeURL, teamName, teamPassword, startDate, endDate, layout)
			if err != nil {
	...
	reminderCmd.Flags().String(flagStartDate, "", "Specify the start date. Format should be 'YYYY-MM-DD HH:MM:SS TZ'. ")
	reminderCmd.Flags().String(flagEndDate, "", "Specify the end date. Format should be 'YYYY-MM-DD HH:MM:SS TZ'.")
